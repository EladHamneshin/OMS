            ...
            steps {
                script {
                    dir('server') {
                        ...
                        sh 'docker build -t oms-end-test . npm run buildtest && npm test'

                        // Start MongoDB container
                        sh 'docker run -d --network app-network --name mongo-db mongo'

                        // Build and run the Express.js server container
                        sh "docker run --rm --name test --network app-network -e MONGO_URI=mongodb://mongo-db:27017/test oms-end-test"
                    }
                }
            }
        }
